...
//日期的加减
const addDay = (date,data) => {
  //下面的不是时间戳，是时间戳*1000
  var timestamp = Date.parse(new Date(Date.parse(date)));
...
//月份第几天增加后获取月份的第几天
const getDayByAddDay = (date,data) => {
  //下面的不是时间戳，是时间戳*1000
  var timestamp = Date.parse(new Date(Date.parse(date)));
  var newTimestamp = timestamp + data * 24 * 60 * 60 * 1000;
  var date = new Date(newTimestamp);
...
const getMonthDayWeek=date=>{
  return formatDateToSimple(date) + ' ' +formatDateToWeek(date)
}
module.exports = {
  ...
  getMonthDayWeek: getMonthDayWeek
}
